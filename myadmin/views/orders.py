...
def index(request,pIndex=1):
    ...
    if kw:
        ...
        list = mod.filter(Q(linkman__contains=kw) | Q(address__contains=kw)).order_by('-addtime')
        mywhere.append("keyword="+kw)
    else:
        list = mod.filter().order_by('-addtime')

    # 获取、判断并封装订单状态state搜索条件
    state = request.GET.get('state','')
    if state != '':
        list = list.filter(state=state).order_by('-addtime')
