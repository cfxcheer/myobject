...
def index(request,pIndex):
    ...
    if kw:
        # 查询商品名中只要含有关键字的都可以
        list = mod.filter(goods__contains=kw).order_by('-id')
        mywhere.append("keyword="+kw)
    else:
        list = mod.filter().order_by('-id')
    ...
    if typeid != '0':
        ...
        list = list.filter(typeid__in=tids).order_by('-id')
        mywhere.append("typeid="+typeid)
    # 获取、判断并封装商品状态state搜索条件
    state = request.GET.get('state','')
    if state != '':
        list = list.filter(state=state).order_by('-id')
...
def insert(request):
    try:
        ...
        print(err)
        context = {'info':'添加失败！'}
        os.remove("./static/goods/m_"+filename) #执行新图片删除  
        os.remove("./static/goods/s_"+filename) #执行新图片删除  
